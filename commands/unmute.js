...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        .setColor("RANDOM")
        .setDescription(`!unmute [\`membro\`]`);

        if(!membro) return message.reply({ content: `${message.author}`, embeds: [embed_1] })

        let embed_2 = new Discord.MessageEmbed()
        .setColor("BLUE")
        .setDescription(`**O usuário ${membro} foi demutado com sucesso!**`);
        
        message.reply({ content: `${message.author}`, embeds: [embed_2]}).then(msg => {

            client.guilds.cache.get(servidor).members.cache.get(membro.id).roles.remove(cargo_mute).catch(err => {
